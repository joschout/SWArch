...
\begin{itemize}
    ...
    \begin{itemize}
   	...
   	\item \texttt{Boolean updateDocumentTemplate(SessionId sessionId, CustomerId cuId, DocumentType documentType, Template template) throws NotAuthenticatedException, InvalidDocumentTypeException}
        ...
        \begin{itemize}
            \item Effect: The \texttt{CustomerOrganizationFacade} first verifies the given session identifier \texttt{sessionId} using the \texttt{AuthenticationHandler}. The \texttt{CustomerOrganizationFacade} then requests to get an overview of all the document that the customer organization identified by the given customer organization identifier has ever initiated. It does this by querying the \textbf{JobManager} to get all status messages for each job belonging to the customer organization. It wraps these statuses in a \texttt{JobStatusOverview} object, which it returns.
            \item Exceptions:
             \begin{itemize}
                \item NotAuthenticatedException: Thrown if the given session identifier is invalid.
            \end{itemize}       	
...
\begin{itemize}
    \item EdocsAdminIncoming
        ...
        \begin{itemize}
        	\item Effect: The \texttt{RegistrationManager} asks the \texttt{EDocsAdminClient} for confirmation of the unregistration after verifying the session identifier. If the eDocs Administrator confirms, the account of the customer organization gets marked as inactive. Finally, the \texttt{RegistrationManager} logs the customer organization out.
        	\item Exceptions:
        	\begin{itemize}
        		\item NotAuthenticatedException: Thrown if the given session identifier is invalid.
        	\end{itemize}
