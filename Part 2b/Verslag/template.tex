...
The \texttt{DocumentStorageManager} is responsible for storing generated documents in the correct database(s) (i.e. the \texttt{PDSDB} and the \texttt{DocumentDB}). If the document belongs to a registered recipient, it sends a write request to both the \texttt{DocumentDB} and the \texttt{PDSDB}. However, if the document belongs to an unregistered recipient, it only sends a write request to the \texttt{DocumentDB}. Furthermore, the \texttt{DocumentStorageManager} is responsible for copying documents from the \texttt{DocumentDB} to the \texttt{PDSDB} when an unregistered Recipient registers to the eDocs system. Another responsibility of the \texttt{DocumentStorageManager} is storing at least 3 hours of documents when the PDSDB fails.
...
\begin{itemize}
    \item RecipientMgmt
    \begin{itemize}
        \item \texttt{SessionId getSessionId() throws NotAuthenticatedException}
        \begin{itemize}
            \item Effect: The \texttt{RecipientClient} returns the session identifier of the current session if the recipient is a Registered Recipient and currently logged in. Otherwise, an exception is thrown.
            \item Exceptions:
            \begin{itemize}
                \item NotAuthenticatedException: Thrown if the recipient does not currently have a session with the eDocs system.
            \end{itemize}
		\end{itemize}
