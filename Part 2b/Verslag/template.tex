...
\begin{figure}[!htp]
    \centering
    \includegraphics[width=\textwidth]{Seq_UC3InitiateDocumentProcessing.png}
...
\begin{itemize}
    ...
    \begin{itemize}
    	\item \texttt{List<RawData> generateDocumentsNonRecurring(CustomerId cuid, SessionId sessionId, \\DocumentType docType, RawDataPacket rawDataPacket, BatchMetaData batchMetaData,\\ DocumentPriority priority) throws NotAuthenticatedException, InvalidDocumentTypeException, InvalidRawDataPackageException }
    	 \begin{itemize}
            \item Effect: The \texttt{CustomerOrganizationFacade} gets an indication from a customer organization with customer identifier \texttt{cuid}  to start a nonrecurring batch of document processing jobs. The documents to be generated should be of document type \texttt{docType}. The information necessary to generate the documents is given in  a \texttt{RawDataPacket} containing all the info about the raw data entries of the batch, and the \texttt{batchMetaData}. After verification of the session identifier, the \texttt{CustomerOrganizationFacade} will put a \texttt{TimeStamp} of the time when this method is called in the \texttt{batchMetaData} and forward its arguments to the \texttt{RawDataHandler}, while indicating it is a nonrecurring batch of document that should be generated.
            \item Exceptions:
            \begin{itemize}
            	\item NotAuthenticatedException: Thrown if the given session identifier is invalid.
            	\item InvalidDocumentTypeException: Thrown if the given document type is invalid or not allowed for the given customer organization.
