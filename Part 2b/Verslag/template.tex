...
We want to have the response time of active replication. But actively replicating the whole database might have to high a cost, so we choose sharding. This keeps the fast response time, but has less hardware required.
...
De userfacade markeert ook documenten als received. De PDSFacade en LinkManager DOEN DIT NIET. Voor r
...
%Ook zorgt de component ervoor dat bij het registreren van een Unregistered Recipient --> opschrijven in een verdere decompositie
...
USE CASE 12: residual drivers: het opslaan van de recipient id's in de pdsdb --> hoort bij deliveryfunctionality. 
...
The metadata is saved in both the pdsdb en database components, because when a user registers, this metadata has to be copied form the one to the other. Dit is is om gemakkelijk de documenten in de pdsdb te kunnen zoeken.
...
\begin{itemize}
    \item \textbf{Description:} The \texttt{DocumentGenerationManager} monitors the availability of the \texttt{Generator} using ping/echo. The \texttt{DocumentGenerationManager} keeps track of the jobs assigned to and being processed by the \texttt{Generators}. The \texttt{DocumentGenerationManager} assigns jobs to the \texttt{Generators} in groups of more than one job that are part of the same batch.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \texttt{JobBatch notifyCompletedAndGiveMeMore(GeneratorId id)}
        \begin{itemize}
            \item Effect: Describe the effect of the operation
            \item Exceptions: None
        \end{itemize}
...
\begin{itemize}
    \item \textbf{Description:} Generates the documents and forwards them to \texttt{OtherFunctionality} to store and deliver them.   
    \item \textbf{Super-component:} None
    \item \textbf{Sub-components:} None
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \texttt{void storeAndDeliverDocument(JonId jobid, Document doc) throws SomeException}
        \begin{itemize}
...
\begin{itemize}
	\item \texttt{BatchId}: Description of data type.
    ...
    \item \texttt{GeneratordId}: Description of data type.
    \item \texttt{JobBatch}: Description of data type.
    \item \texttt{JobId}: Description of data type.
    \item \texttt{MetaData}: Description of data type.
